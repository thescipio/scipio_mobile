...
class EditIssuePage extends StatefulWidget {
  ...
  final String initialAttachmentLink;
  ...
  const EditIssuePage({
    ...
    required this.initialAttachmentLink,
  });
...
class _EditIssuePageState extends State<EditIssuePage> {
  ...
  void initState() {
    ...
    detailController = TextEditingController(text: widget.initialDescription);
    linkController = TextEditingController(text: widget.initialAttachmentLink);
    fetchDevices();
    fetchVersions();
  }
  ...
  Future<void> fetchDevices() async {
    try {
      final response = await Dio().get('https://canna.hlcyn.co/api/device');
      setState(() {
        ...
        if (devices
            .any((device) => device['codename'] == widget.initialDevice)) {
  ...
  Future<void> fetchVersions() async {
    try {
      final response = await Dio().get('https://canna.hlcyn.co/api/version');
      setState(() {
        ...
        if (versions
            .any((version) => version['branch'] == widget.initialVersion)) {
  ...
  Future<void> submitIssue() async {
    try {
      ...
      print('Issue update response: ${response.data}');
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Text('Edit Issue', style: TextStyle(fontSize: 24)),
